    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="card p-6">
                            ...
                            <div class="flex-1">
                                ...
                                @if ($feedback->status === 'open')
                                    <div class="flex gap-2 mt-2">
                                        <form method="POST" action="{{ route('dean.feedback.resolve', $feedback) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs btn-primary !px-3 !py-1">Resolve</button>
                                        </form>
                                        <form method="POST" action="{{ route('dean.feedback.escalate', $feedback) }}" class="inline">
                                            @csrf
                                            <button type="submit" class="text-xs btn-secondary !px-3 !py-1">Escalate to SAO</button>
                                        </form>
                                    </div>
                                @endif
                            </div>
                ...
                @empty
                    <p class="text-center text-gray-400 py-8">No feedback for your college yet.</p>
                @endforelse
                <div class="mt-4">
                    {{ $feedbacks->links() }}
                </div>
